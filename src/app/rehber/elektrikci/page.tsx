...
import { electricianData } from '@/data/electrician-data';
...
export default function ElectricianGuidePage() {
  const { title, description, sections, faq, stats, videoUrl: defaultVideoUrl, analogy } = electricianData;
  ...
  useEffect(() => {
    async function fetchPageData() {
      ...
      const { data: expData } = await supabase
        ...
        .select('*')
        .eq('status', 'approved')
        .or('profession.ilike.%Elektrik%,profession.ilike.%Elektronik%,profession.ilike.%Electrician%')
  ...
  const getIconForSection = (id: string) => {
    switch (id) {
      case 'temel-meslekler': return <Briefcase className="w-6 h-6 text-blue-600" />;
      case 'denklik-surecleri': return <FileText className="w-6 h-6 text-orange-600" />;
      case 'farklilik-alanlari': return <ShieldCheck className="w-6 h-6 text-red-600" />;
      case 'egitim-icerigi': return <GraduationCap className="w-6 h-6 text-green-600" />;
            ...
            <div className="flex-1 space-y-4 w-full">
              ...
              <div className="flex justify-center md:justify-start gap-3 pt-4">
                ...
                <ShareExperienceDialog 
                  ...
                  defaultProfessionName="Elektrikçi / Elektronikçi" 
                />
              </div>
            </div>
