...
import { itData } from '@/data/it-data';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { CheckCircle2, BookOpen, PlayCircle, GraduationCap, User, Code2, Shield, Database, Server, Terminal, Cpu, ArrowRight } from 'lucide-react';
...
// Helper to get icon for specialization
function getSpecializationIcon(title: string) {
  if (title.includes('Security')) return Shield;
  if (title.includes('Data')) return Database;
  if (title.includes('System') || title.includes('DevOps')) return Server;
  if (title.includes('Tester')) return CheckCircle2;
  if (title.includes('SAP')) return Cpu;
  return Code2; // Default for Developer
}
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
            <div className="flex-1 space-y-4 w-full">
              <div className="flex items-center justify-center md:justify-start gap-2">
                <Badge variant="outline" className="text-blue-600 border-blue-200 bg-blue-50">Teknoloji</Badge>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                {itData.stats.map((stat: { label: string; value: string; color: string }, index: number) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-white font-semibold text-sm">IT Bilişim Grubu</div>
              ...
              <div className="flex justify-center md:justify-start gap-3 pt-4">
                ...
                <ShareExperienceDialog professionSlug="bilisim-it" />
              </div>
            </div>
        ...
        <Tabs defaultValue="roadmap" className="space-y-8">
          ...
          <TabsContent value="specializations">
            <div className="grid md:grid-cols-2 gap-6">
              {itData.specializations?.map((spec, index) => {
                const Icon = getSpecializationIcon(spec.title);
                
                const CardContentWrapper = (
                  <Card className={`h-full hover:shadow-md transition-all duration-200 ${spec.link ? 'hover:border-blue-500 hover:shadow-lg cursor-pointer group' : ''}`}>
                    <CardHeader>
                      <div className="flex items-center justify-between mb-2">
                        <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                          <Icon className="w-6 h-6 text-blue-600 dark:text-blue-400" />
                        </div>
                        <Badge variant={spec.demand === 'Çok Yüksek' ? 'destructive' : 'default'}>
                          {spec.demand} Talep
                        </Badge>
                      </div>
                      <CardTitle className="text-xl flex items-center gap-2">
                        {spec.title}
                        {spec.link && <ArrowRight className="w-5 h-5 text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity -translate-x-2 group-hover:translate-x-0 duration-200" />}
                      </CardTitle>
                      <CardDescription>{spec.description}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div>
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2">Gerekli Yetkinlikler</h4>
                      ...
                      <div className="pt-2 border-t">
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-slate-500">Ortalama Maaş:</span>
                          <span className="font-semibold text-slate-900 dark:text-slate-100">{spec.salary}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                if (spec.link) {
                  return (
                    <Link key={index} href={spec.link} className="block h-full">
                      {CardContentWrapper}
                    </Link>
                  );
                }

                return <div key={index} className="h-full">{CardContentWrapper}</div>;
              })}
            </div>
          </TabsContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {itData.pedagogy.title}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="prose dark:prose-invert max-w-none">
                  <p className="text-lg leading-relaxed text-slate-700 dark:text-slate-300">
                    {itData.pedagogy.content}
                  </p>
          ...
          <TabsContent value="faq">
            <FaqSection professionSlug="bilisim-it" initialFaqs={itData.faq} />
          </TabsContent>

          <TabsContent value="experiences">
            <ExperienceSection professionSlug="bilisim-it" />
          </TabsContent>
          ...
          <TabsContent value="documents">
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <UploadDocumentDialog professionSlug="bilisim-it" />
              </div>
              <DocumentSection professionSlug="bilisim-it" />
            </div>
          </TabsContent>
        </Tabs>
      </div>
