...
import { physicalEducationTeacherData } from '@/data/physical-education-teacher-data';
...
export default function PhysicalEducationTeacherPage() {
  const [videoUrl, setVideoUrl] = useState(physicalEducationTeacherData.videoUrl);
  const [pageTitle, setPageTitle] = useState(physicalEducationTeacherData.title);
  const [pageDescription, setPageDescription] = useState(physicalEducationTeacherData.description);
  const [activeTab, setActiveTab] = useState('roadmap');
  const { updates } = useCommunityUpdates('beden-egitimi-ogretmenligi');
  ...
  useEffect(() => {
    async function fetchPageData() {
      const { data } = await supabase
        ...
        .eq('slug', 'beden-egitimi-ogretmenligi')
        .single();
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
            <div className="flex-1 space-y-4 w-full">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                {physicalEducationTeacherData.stats.map((stat, index) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-6">
                <a 
                  href="https://t.me/+8D4PF0l5w8Y3NmMy" 
                  target="_blank" 
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-white font-semibold text-sm">Beden Eğitimi Öğretmenleri</div>
              ...
              <div className="flex justify-center md:justify-start gap-3 pt-4">
                ...
                <ShareExperienceDialog professionSlug="beden-egitimi-ogretmenligi" />
              </div>
            </div>
      ...
      <div id="roadmap-section" className="container mx-auto px-4 py-8 max-w-5xl">
        <PageCommunityUpdatesBanner 
          categorySlug="beden-egitimi-ogretmenligi"
          groupName="BEDEN EĞİTİMİ ÖĞRETMENLERİ" 
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-6 lg:w-[860px] h-auto p-1 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl">
            ...
            <TabsTrigger value="pedagogy" className="py-2.5 rounded-lg">Pedagoji</TabsTrigger>
          ...
          <TabsContent value="updates">
            <PageCommunityUpdatesContent 
              ...
              groupName="BEDEN EĞİTİMİ ÖĞRETMENLERİ"
              telegramUrl="https://t.me/+8D4PF0l5w8Y3NmMy" 
            />
          </TabsContent>
          ...
          <TabsContent value="roadmap" className="space-y-8">
            <div className="grid gap-8">
              {physicalEducationTeacherData.roadmap.map((step, index) => (
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <Card className="md:col-span-2">
                ...
                <CardContent className="space-y-6">
                  <div>
                    <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-2">Temel Yaklaşımlar</h4>
                    <p className="text-slate-600 dark:text-slate-300">
                      {physicalEducationTeacherData.pedagogy.content}
                    </p>
                  </div>
                  
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-md border border-yellow-100 dark:border-yellow-900/50">
                    <h4 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-2">Kapsayıcı Eğitim (Inklusion)</h4>
                    <p className="text-sm text-yellow-700 dark:text-yellow-300">
                      {physicalEducationTeacherData.pedagogy.specialNeeds}
                    </p>
                  </div>

                  <div>
                     <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-2">Ders Yapısı</h4>
                     <p className="text-sm text-slate-600 dark:text-slate-400">
                       {physicalEducationTeacherData.pedagogy.resources}
                     </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-4">
                    {physicalEducationTeacherData.alternatives.map((alt, idx) => (
                ...
                <CardContent>
                  <ul className="space-y-3">
                    <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                      ...
                      <span>Mülakata asla hazırlanmadan gitmeyin. Prova yapın.</span>
                    </li>
                    <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 shrink-0" />
                      <span>Türkiye'deki tecrübelerinizi ve başarılarınızı detaylıca anlatın.</span>
                    </li>
                    <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 shrink-0" />
                      <span>Sınıf yönetimi ve veli iletişimi konularında somut örnekler verin.</span>
                    </li>
                  </ul>
                </CardContent>
              </Card>
          ...
          <TabsContent value="faq">
            <FaqSection professionSlug="beden-egitimi-ogretmenligi" />
          </TabsContent>
          ...
          <TabsContent value="experiences">
            <div className="space-y-6">
              ...
              <ExperienceSection professionSlug="beden-egitimi-ogretmenligi" />
            </div>
          </TabsContent>
          ...
          <TabsContent value="documents">
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-xl border border-indigo-100 dark:border-indigo-800">
                ...
                <UploadDocumentDialog professionSlug="beden-egitimi-ogretmenligi" />
              </div>
              
              <DocumentSection professionSlug="beden-egitimi-ogretmenligi" />
            </div>
          </TabsContent>
        </Tabs>
      </div>
