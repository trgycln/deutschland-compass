...
import { mathTeacherData } from '@/data/math-teacher-data';
...
export default function MathTeacherPage() {
  const [videoUrl, setVideoUrl] = useState(mathTeacherData.videoUrl);
  const [pageTitle, setPageTitle] = useState(mathTeacherData.title);
  const [pageDescription, setPageDescription] = useState(mathTeacherData.description);
  const [activeTab, setActiveTab] = useState('roadmap');
  const { updates } = useCommunityUpdates('matematik-ogretmenligi');
  ...
  useEffect(() => {
    async function fetchPageData() {
      const { data } = await supabase
        ...
        .eq('slug', 'matematik-ogretmenligi')
        .single();
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
            <div className="flex-1 space-y-4 w-full">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                {mathTeacherData.stats.map((stat, index) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-6">
                <a 
                  href="https://t.me/+4kz3L1t4DzY2MDUy" 
                  target="_blank" 
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-white font-semibold text-sm">Matematik Öğretmenleri</div>
              ...
              <div className="flex justify-center md:justify-start gap-3 pt-4">
                ...
                <ShareExperienceDialog professionSlug="matematik-ogretmenligi" />
              </div>
            </div>
            ...
            <div className="w-full md:w-1/3 aspect-video bg-slate-200 rounded-xl overflow-hidden shadow-lg border border-slate-200 dark:border-slate-800">
              {videoUrl.includes('youtube') || videoUrl.includes('youtu.be') ? (
                ...
                <video 
                  controls 
                  poster="/images/video-poster.jpg" // İsteğe bağlı poster görseli
      ...
      <div id="roadmap-section" className="container mx-auto px-4 py-8 max-w-5xl">
        <PageCommunityUpdatesBanner 
          categorySlug="matematik-ogretmenligi"
          groupName="MATEMATİK ÖĞRETMENLERİ GRUBU" 
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          ...
          <TabsContent value="updates">
            <PageCommunityUpdatesContent 
              ...
              groupName="MATEMATİK ÖĞRETMENLERİ GRUBU"
              telegramUrl="https://t.me/+4kz3L1t4DzY2MDUy" 
            />
          </TabsContent>
          ...
          <TabsContent value="roadmap" className="space-y-8">
            <div className="grid gap-8">
              {mathTeacherData.roadmap.map((step, index) => (
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <Card className="md:col-span-2">
                ...
                <CardContent className="space-y-6">
                  <div>
                    <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-2">Ders İşleyişi</h4>
                    <p className="text-slate-600 dark:text-slate-300">
                      {mathTeacherData.pedagogy.content}
                    </p>
                  </div>
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-md border border-yellow-100 dark:border-yellow-900/50">
                    ...
                    <p className="text-sm text-yellow-700 dark:text-yellow-300">
                      {mathTeacherData.pedagogy.specialNeeds}
                    </p>
                  </div>
                  ...
                  <div>
                     ...
                     <p className="text-sm text-slate-600 dark:text-slate-400">
                       {mathTeacherData.pedagogy.resources}
                     </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-4">
                    {mathTeacherData.alternatives.map((alt, idx) => (
                ...
                <CardContent>
                  <ul className="space-y-3">
                    <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                      ...
                      <span>Pedagojik sorularda "kahraman" olmayın. İşbirliğine açık olduğunuzu gösterin.</span>
                    </li>
                    <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 shrink-0" />
                      <span>Kendi hazırladığınız bir ders planını yanınızda götürün.</span>
                    </li>
                    <li className="flex gap-3 text-sm text-slate-600 dark:text-slate-300">
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 shrink-0" />
                      <span>Motivasyon mektubunda ChatGPT'den faydalanın ama kişiselleştirin.</span>
                    </li>
                  </ul>
                </CardContent>
              </Card>
          ...
          <TabsContent value="faq">
            <FaqSection professionSlug="matematik-ogretmenligi" />
          </TabsContent>
          ...
          <TabsContent value="experiences">
            <div className="space-y-6">
              ...
              <ExperienceSection professionSlug="matematik-ogretmenligi" />
            </div>
          </TabsContent>
          ...
          <TabsContent value="documents">
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-xl border border-indigo-100 dark:border-indigo-800">
                ...
                <UploadDocumentDialog professionSlug="matematik-ogretmenligi" />
              </div>
              
              <DocumentSection professionSlug="matematik-ogretmenligi" />
            </div>
          </TabsContent>
        </Tabs>
      </div>
