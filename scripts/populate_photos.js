...
// Extensive, verified, high-resolution culinary photography library
const DONER_PHOTOS = [
...
function resolvePhoto(place) {
  const text = `${place.food || ""} ${place.name || ""} ${place.category || ""}`.toLowerCase();
  const seed = `${place.id || ""}_${place.name || ""}_${place.city || ""}`;

  if (/döner|doner|kebap|kebab|iskender|adana|urfa|köfte|kofte|dürüm|durum/.test(text)) return pickByHash(DONER_PHOTOS, seed);
  if (/tantuni|dürüm|wrap|dürümü/.test(text)) return pickByHash(WRAP_PHOTOS, seed);
  if (/pide|lahmacun|pizza|taş fırın/.test(text)) return pickByHash(PIDE_PHOTOS, seed);
  if (/çiğköfte|cigkofte|çiğ köfte|vegan|meze|salata/.test(text)) return pickByHash(MEZE_PHOTOS, seed);
  if (/kahvaltı|kahvalti|çay|kahve|coffee|simit|poğaça|cafe|kafe/.test(text) || place.category === 'cafe') return pickByHash(KAHVALTI_PHOTOS, seed);
  if (/baklava|tatlı|tatli|fırın|firin|pastane|pasta|künefe|dondurma|gelato/.test(text) || place.category === 'bakery') return pickByHash(BAKERY_PHOTOS, seed);
  if (/burger|chicken|tavuk|hfc|kfc|crispy|patates|fast food/.test(text) || place.category === 'fast_food') return pickByHash(BURGER_PHOTOS, seed);
  if (/kasap|metzgerei|et|kıyma|kasabı/.test(text) || place.category === 'butcher') return pickByHash(BUTCHER_PHOTOS, seed);
  if (/market|süpermarket|supermarkt|bakkal|feinkost/.test(text) || place.category === 'market') return pickByHash(MARKET_PHOTOS, seed);
  return pickByHash(RESTAURANT_PHOTOS, seed);
}

async function run() {
  const { data: places, error } = await supabase.from('places').select('id, name, city, category, food');
  if (error) { console.error('Fetch error:', error); return; }
  console.log(`Distributing diverse, authentic photos to ${places.length} places...`);
  ...
  for (let i = 0; i < places.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(p => {
      ...
      return supabase.from('places').update({ foto_url: url }).eq('id', p.id);
    }));
    process.stdout.write(`Updated ${Math.min(i + BATCH_SIZE, places.length)} / ${places.length} photos...\r`);
  }
  console.log('\nDONE! All places have been successfully updated with diverse photography.');
}
